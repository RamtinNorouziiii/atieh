...
let menu;
export default function WithSubnavigation({ data }: any) {
...
const DesktopSubNav = ({ title, webLink }: any) => {
  return (
    <Link
      href={webLink}
      role={'group'}
  ...
  return (
    <Stack
      ...
      {NAV_ITEMS.map((navItem) => (
        <MobileNavItem key={navItem.label} {...navItem} data={data} />
      ))}
    </Stack>
  );
};
        ...
        <Stack
          ...
          {data &&
            data.data.map((
              //@ts-ignore
              child: any) => (
              <Link key={child.title} py={2} href={child.webLink}>
                {child.title}
              </Link>
            ))}
        </Stack>
